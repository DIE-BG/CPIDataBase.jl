...
function (inflfn::InflationFunction)(cs::CountryStructure, cpi_result::CPIIndex)
    inflfn(cs, cpi_result, nothing)
end

function (inflfn::InflationFunction)(cs::CountryStructure, cpi_result::CPIVarInterm)
    inflfn(cs, cpi_result, nothing)
end

function (inflfn::InflationFunction)(cs::CountryStructure, date::Union{Date, Nothing})
    cpi_index = inflfn(cs, CPIIndex(), date)
    varinteran(cpi_index)
end

function (inflfn::InflationFunction)(cs::CountryStructure, ::CPIIndex, date::Union{Date, Nothing})
    v_interm = inflfn(cs, CPIVarInterm(), date)
    capitalize!(v_interm, 100) # v_interm -> cpi_index
    v_interm  
end

function (inflfn::InflationFunction)(cs::CountryStructure, ::CPIVarInterm, date::Union{Date, Nothing}) 
